...
public class GearDao {
	...
	public Gear create(Gear gear) throws SQLException  {
		String insertGear =
				"INSERT INTO gear(item_id,item_level,gear_slot_id,required_level,defense_rating,magic_defense_rating) " +
				"VALUES(?,?,?,?,?,?);";
		...
		public Gear getGearByID(int itemID) throws SQLException {
			String selectGear =
					"SELECT g.item_id as item_id,i.item_name as item_name,i.max_stack_size as max_stack_size,i.vendor_price as vendor_price,g.item_level as item_level,g.gear_slot_id as gear_slot_id,g.required_level as required_level,g.defense_rating as defense_rating,g.magic_defense_rating as magic_defense_rating " +
					"FROM Gear g INNER JOIN Item i " +
				...
				try {
					...
					if(results.next()) 
					{
						...
						Gear gear = new Gear(itemID,resultItemName,resultMaxStackSize,resultVendorPrize,resultItemLevel,resultGearSlotID,resultRequiredLevel,resultDefenseRating,resultMagicDefenseRating);
						return gear;
					}
				} catch (SQLException e) {
					e.printStackTrace();
	    ...
	    public List<Gear> getGearByPartialName(String name) throws SQLException {
	        ...
	        String selectGearByPartialName =
	                "SELECT g.item_id, i.item_name, i.max_stack_size, i.vendor_price, g.item_level, " +
	                        "g.gear_slot_id, g.required_level, g.defense_rating, g.magic_defense_rating " +
	                        "FROM gear g INNER JOIN item i ON g.item_id = i.item_id " +
	        ...
	        try {
	            ...
	            while (results.next()) {
	                ...
	                Gear gear = new Gear(itemID, itemName, maxStackSize, vendorPrice, itemLevel, gearSlotID,
	                        requiredLevel, defenseRating, magicDefenseRating);
	                gears.add(gear);
	            }
	        } catch (SQLException e) {
	            e.printStackTrace();
	    ...
	    public Gear delete(Gear gearItem) throws SQLException {
	        String deleteGear =
	                "DELETE FROM gear WHERE item_id = ?;";
	        Connection connection = null;
