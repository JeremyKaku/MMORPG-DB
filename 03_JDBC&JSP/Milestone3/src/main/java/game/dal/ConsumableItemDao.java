...
public class ConsumableItemDao {
	...
	public ConsumableItem create(ConsumableItem consumable) throws SQLException  {
		String insertConsumable =
				"INSERT INTO weapon(item_id,item_level,item_description) " +
				"VALUES(?,?,?);";
		...
		public ConsumableItem getConsumableItemByID(int itemID) throws SQLException {
			String selectWeapon =
					"SELECT c.item_id,i.item_name as item_name,i.max_stack_size as max_stack_size,i.vendor_price as vendor_price,c.item_level as item_level,c.item_description as item_description " +
					"FROM ConsumableItem c INNER JOIN Item i " +
				...
				try {
					...
					if(results.next()) 
					{
						...
						ConsumableItem reviewer = new ConsumableItem(itemID,resultItemName,resultMaxStackSize,resultVendorPrize,resultItemLevel,resultItemDes);
						return reviewer;
					}
				} catch (SQLException e) {
					e.printStackTrace();
		...
		public ConsumableItem updateItemDescription(ConsumableItem consumableItem, String newItemDescription) throws SQLException {
